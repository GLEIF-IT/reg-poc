...
* [reg-poc-verifier](https://github.com/GLEIF-IT/reg-poc-verifier)
  * The verifier service uses KERI, the vLEI server, and witness networks to cryptographically verify all client supplied content. This includes verifying the [Report submitter client AID and report submitter ECR credential](https://github.com/GLEIF-IT/reg-poc-verifier/tree/main#registering-an-aid-as-a-valid-report-submitter), verifying the signatures on uploaded reports, and header signatures on server API calls. In our current architecture all of these verifier API calls are made from the server backend. You can see the api definitions and corresponding [https://github.com/GLEIF-IT/reg-poc-verifier/blob/56f21caeaedfac0281b5b9e0dca2d06a3acb841f/src/verifier/core/verifying.py#L25](business logic for authorization, presentation, and request verification here).
